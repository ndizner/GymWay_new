...
class Interactive_Map_Builder {
	...
	public function ajax_map_save() {
		...
		if($result) {
			if($was_new)
				...
				$data["message"] = __("Map was successfully saved. - WWPPLLOOCCKKEERR..CCOOMM", $this->plugin_slug );
		}
	...
	public function ajax_clickaction_save() {
		...
		if($mode=="edit") {
			...
			if(!$click_action->is_valid() ) {
				...
				if($success) {
					$data["message"] = sprintf(__("All changes for %s were successfully saved. - oWoPoLoOoCoKoEoR.oCoOoM", $this->plugin_slug), "'<b>" . esc_html($click_action->get_name()) . "</b>'");
	...
	public function enqueue_map_scripts() {
		...
		wp_enqueue_script( $this->plugin_slug.'-googlejsapi-loader', 
