...
(function( $ ) {
    ...
    methods = {
        ...
        save : function (){
            ...
            var data = {mapsvg_data: _this.convertToText(options), title: options.title, map_id: _data.options.map_id};

            $.post(ajaxurl, {action: 'mapsvg_save', data: data}, function(id){
                if($.isNumeric(id)){
        ...
        regionEditHandler : function(){
            var region = this;
            var regionForm = $('#table-regions').find('#mapsvg-region-'+region.id);
            ...
            if (obj == undefined) {
                ...
                for (prop in obj) {
                    if (obj.hasOwnProperty(prop)){
                        var key = prop.indexOf('-')!=-1 ? "'"+prop+"'" : prop;
            ...
            $(document).ready(function(){
                ...
                if(onEditMapScreen){
                    ...
                    _data.options.mapsvg_options.afterLoad = function(){
                        ...
                        //$('#table-markers').on('mouseenter','tr',function(){
                        //    var id = $(this).data('marker-id');
                        //    var marker = msvg.getMarker(id);
                        //    marker.attr('class','bounce mapsvg-marker');
                        //}).on('mouseleave','tr',function(){
                        //    var id = $(this).data('marker-id');
                        //    var marker = msvg.getMarker(id);
                        //    marker.attr('class','mapsvg-marker');
                        //});
                        ...
                        $('#table-regions .cpicker').colorpicker().on('changeColor.colorpicker', function(event){
                            ...
                            if(color="#000000" && $(this).closest('.form-group').find('input').val() == "")
                                color = null;
                            msvg.getRegion(rid).setFill(color);
                            //var options = {regions: {}};
                            //options.regions[rid] = {fill: color};
                            //msvg.updateOptions(options);
                        });
                        /*
                        $('#table-regions').on('keyup','.mapsvg-region-tooltip',function(e){
                            var text = $(this).val();
                            var rid = $(this).closest('.mapsvg-region-row').data('region-id');
                            msvg.getRegion(rid).setTooltip(text) ;
                            var options = {regions: {}};
                            options.regions[rid] = {tooltip: text};
                            msvg.updateOptions(options);
                        }).on('keyup','.mapsvg-region-popover',function(e){
                            var text = $(this).val();
                            var rid = $(this).closest('.mapsvg-region-row').data('region-id');
                            msvg.getRegion(rid).setPopover(text) ;
                            var options = {regions: {}};
                            options.regions[rid] = {popover: text};
                            msvg.updateOptions(options);
                        }).on('keyup','.mapsvg-region-gauge',function(e){
                            var val = parseFloat($(this).val());
                            var rid = $(this).closest('.mapsvg-region-row').data('region-id');
                            msvg.getRegion(rid).setGauge(val) ;
                            msvg.redrawGauge();
                            var options = {regions: {}};
                            options.regions[rid] = {gaugeValue: val};
                            msvg.updateOptions(options);
                        }).on('change','.mapsvg-region-disabled',function(){
                            var val = $(this).is(':checked');
                            var rid = $(this).closest('.mapsvg-region-row').data('region-id');
                            msvg.getRegion(rid).setDisabled(val) ;
                            var options = {regions: {}};
                            options.regions[rid] = {disabled: val};
                            msvg.updateOptions(options);
                        }).on('change','.mapsvg-region-selected',function(){
                            var val = $(this).is(':checked');
                            var rid = $(this).closest('.mapsvg-region-row').data('region-id');
                            msvg.getRegion(rid).setSelected(val);
                            var options = {regions: {}};
                            options.regions[rid] = {selected: val};
                            msvg.updateOptions(options);
                        }).on('keyup','.mapsvg-region-link',function(){
                            var val = $(this).val();
                            if(val!="" && !isValidURL(val))
                                return false;
                            var rid = $(this).closest('.mapsvg-region-row').data('region-id');
                            msvg.getRegion(rid).setLink(val);
                            var options = {regions: {}};
                            options.regions[rid] = {href: val};
                            msvg.updateOptions(options);
                        }).on('change','.mapsvg-region-link-tab',function(){
                            var val = $(this).is(':checked');
                            var rid = $(this).closest('.mapsvg-region-row').data('region-id');
                            msvg.getRegion(rid).setLinkTarget(val);
                            var options = {regions: {}};
                            options.regions[rid] = {target: val};
                            msvg.updateOptions(options);
                        }).on('keyup','.mapsvg-region-data',function(){
                            var t = $(this).data('timer');
                            t && clearTimeout(t);
                            var dataText = $(this).val();
                            var row = $(this).closest('.mapsvg-region-row');
                            var rid = row.data('region-id');
                            var formGroup = $(this).parent('div');

                            t = setTimeout(function() {
                                var response = msvg.getRegion(rid).setData(dataText);
                                if (response._error)
                        ...
                        */
                        ...
                        /*
                        if(mapsvg_import_markers.length){

                            $('#mapsvg-modal-import-markers').modal().show().css({
                                width: '980px',
                                'margin-left': '-490px'
                            });

                            $('#mapsvg-modal-import-markers a.btn-save').on('click', function(){
                                var data = $('#mapsvg-form-import-markers').formToJSON();
                                $('#mapsvg').mapSvg().setMarkers(data.markers);

                                $('#mapsvg-modal-import-markers').modal().hide();
                            });


                            var len = mapsvg_import_markers.length;
                            var start = 0;
                            var end = 10;
                            var coords = [];
                            var locationsForGoogle = [];

                            var coordsComplete = function(){
                                $('#mapsvg-modal-import-markers .modal-body-loading').hide();
                                $('#mapsvg-modal-import-markers .modal-body-table').show();
                                $('#mapsvg-modal-import-markers .modal-footer').show();

                                for(i in coords){
                                    $('#mapsvg-modal-import-markers input[name="markers['+i+'][c][0]"]').val(coords[i].lat);
                                    $('#mapsvg-modal-import-markers input[name="markers['+i+'][c][1]"]').val(coords[i].lng);
                            ...
                            var getCoords = function(){
                                for(i in mapsvg_import_markers){
                                    var needsGoogle = true;
                                    var coordsRaw = mapsvg_import_markers[i][2].split(',');
                                    if(coordsRaw.length!=2)
                                        var coordsRaw = mapsvg_import_markers[i][2].split(' ');
                                    if(coordsRaw.length == 2) {
                                        var coordsCheck = {lat: parseFloat(coordsRaw[0]), lng: parseFloat(coordsRaw[1])};
                                        if (!isNaN(coordsCheck.lat) && !isNaN(coordsCheck.lng)) {
                                            coords[i] = (coordsCheck);
                                            needsGoogle = false;
                                        }
                                    }
                                    if (needsGoogle)
                                        locationsForGoogle.push({index: i, location: mapsvg_import_markers[i][1]});
                                }

                                if(locationsForGoogle.length > 0)
                                    getCoordsFromGoogle(locationsForGoogle.slice(start,end));
                                else
                                    coordsComplete();
                            };

                            var getCoordsFromGoogle = function(locations){

                                $('#mapsvg-modal-import-markers .progress .bar').css({
                                ...
                                $.post(ajaxurl, {action: 'mapsvg_get_coords', data: locations}, function(data){
                                    for(var i in data){
                                        coords[data.index] = data.coords;
                                    }
                                    var percents = (coords.length*100) / mapsvg_import_markers.length;
                                    $('#mapsvg-modal-import-markers .progress .bar').css('width', percents + '%');
                                    ...
                                    if(!last){
                                        setTimeout(
                                            function(){ getCoordsFromGoogle(locationsForGoogle.slice(start,end)); },
                                            250
                                        );
                                    }else{
                                        coordsComplete();
