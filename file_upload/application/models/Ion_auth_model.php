...
/*
	public function add_role($group_ids, $user_id=false){
		$this->trigger_events('add_role');
		...
		$array = array(
			...
			$this->db->insert($this->tables['roles_groups'],
	}

*/


	public function add_role($role_ids, $user_id=false){
		$this->trigger_events('add_role');

		// si no recibo user id, tomo el de la sesion actual
		$user_id || $user_id = $this->session->userdata('user_id');
		...
		foreach ($role_ids as $role_id){
			...
			$array = array(
				...
				if ($this->db->insert('roles_groups', $array)) {
	...
	public function remove_role($group_ids=false, $user_id=false){
		// user id is required
		if(empty($user_id)){
			return FALSE;
		}
		...
		if( ! empty($group_ids)){
			...
			foreach($group_ids as $group_id){
				$this->db->delete($this->tables['roles_groups'], array($this->join['roles'] => (float)$group_id, $this->join['users'] => (float)$user_id));
		...
		// otherwise remove user from all groups
			...
			if ($return = $this->db->delete($this->tables['roles_groups'], array($this->join['users'] => (float)$user_id))) {
				$this->_cache_user_in_group[$user_id] = array();
			}
		}
		return $return;
